from .helpers import FINGERPRINT, OTHER_FINGERPRINT, create_group, post_event
...
def test_delete_event_success(client):
    ...
    r = client.delete(
        ...
        headers={"X-Fingerprint-ID": created["fingerprint"]},
    )
    assert r.status_code == 200
    # Confirm it's gone
    assert client.get(f"/calendar/events/{created['id']}").status_code == 404
...
def test_get_events_filters_by_group(client):
    elders_group = create_group(client, missionary_type="elders")
    sisters_group = create_group(client, missionary_type="sisters")
    post_event(client, group_id=elders_group)
    post_event(client, group_id=sisters_group)
    r = client.get(f"/calendar/events?group_id={elders_group}")
    assert r.status_code == 200
    data = r.get_json()
    assert len(data) == 1
    assert data[0]["missionary_group"] == elders_group
...
def test_post_event_missing_group_id(client):
    r = client.post(
        "/calendar/events",
        json={
...
def test_post_event_invalid_group_id(client):
    r = client.post(
        "/calendar/events",
        json={
            "phone_num": "0400000000",
